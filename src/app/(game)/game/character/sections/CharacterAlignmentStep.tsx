...
import { Race, SubRace, CharacterStats, AlignmentOption } from '@/types/character';

interface CharacterAlignmentStepProps {
  onRandomize?: () => void;
...
const SCROLL_AREA_HEIGHT = 420;
const THUMB_HEIGHT = 71;

const CharacterAlignmentStep: React.FC<CharacterAlignmentStepProps> = ({
  onRandomize,
  ...
  const handleSelect = (id: string) => {
    if (onSelectAlignment) {
      onSelectAlignment(id);
    } else {
      setInternalSelected(id);
    }
  };

  const selectedOption = alignmentOptions.find((o) => o.id === selected) ?? null;
  const alignmentLabel = selectedOption?.name ?? 'Выберите мировоззрение';
  const alignmentDescription = selectedOption?.description ?? '';
  ...
  return (
    <>
      ...
      >
              ...
              {alignmentOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`Выбрать мировоззрение: ${option.name}`}
                  >
                    <div className="relative w-23 h-19">
                      {isSelected ? (
                        <ClassIconInline
                          src={BOX_ICON}
                          selected
        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-142 flex flex-col items-center text-center max-w-[90%] gap-1">
          <span className="font-firenight text-[25px] text-[#FFEED5] text-nowrap">
            {alignmentLabel}
          </span>
          {alignmentDescription && (
            <p className="font-jost font-normal text-[14px] leading-tight text-white/80 line-clamp-3">
              {alignmentDescription}
            </p>
          )}
        </div>
...
export default CharacterAlignmentStep;
