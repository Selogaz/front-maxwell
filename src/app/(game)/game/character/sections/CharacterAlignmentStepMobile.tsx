...
import { StepId, CharacterSelection, CharacterStats, AlignmentOption } from '@/types/character';

interface CharacterAlignmentStepMobileProps {
  ...
  onSelectAlignment?: (alignmentId: string) => void;
  onNext?: () => void;
  onPrev?: () => void;
  currentStep: StepId;
...
const GENERIC_ICON = `${ORIGIN_ASSET}/мудрец_inner.svg`;
...
const CharacterAlignmentStepMobile: React.FC<CharacterAlignmentStepMobileProps> = ({
  selection,
  totalStats,
  primaryStat,
  alignmentOptions = [],
  onRandomize,
  selectedAlignment: controlledSelectedAlignment,
  onSelectAlignment,
  onNext,
  onPrev,
  currentStep,
  ...
  const selected = controlledSelectedAlignment ?? internalSelected;

  const handleSelect = (id: string) => {
    if (onSelectAlignment) onSelectAlignment(id);
          ...
          <div className="relative p-4 pt-5">
            ...
            <div className="flex flex-col items-center text-center mt-4 gap-1 px-3">
              <span className="font-firenight text-[20px] text-[#FFEED5]">
                {alignmentOptions.find((a) => a.id === selected)?.name ?? 'Выберите мировоззрение'}
              </span>
              {alignmentOptions.find((a) => a.id === selected)?.description && (
...
export default CharacterAlignmentStepMobile;
