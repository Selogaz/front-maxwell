...
import { StepId, CharacterSelection, CharacterStats, ClassOption } from '@/types/character';
import { getClassIcon } from '@/services/classes';

interface CharacterClassStepMobileProps {
...
const CharacterClassStepMobile: React.FC<CharacterClassStepMobileProps> = ({
  selection,
  totalStats,
  primaryStat,
  onRandomize,
    ...
    <div className="md:hidden flex flex-col items-center w-full pb-8 pt-2.5 bg-[#020106]">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-firenight text-[22px] text-[#FFEED5] leading-6 font-normal font-firenight pb-4 pl-2">КЛАСС</h2>
          <button
            type="button"
            onClick={onRandomize}
            aria-label="Случайный класс"
        ...
        >
          <div className="grid grid-cols-3 gap-1 -mt-2 pr-6">
            {classOptions.length === 0 ? (
              <div className="col-span-3 text-center text-[#ECECEC] font-jost text-[13px] py-4">Загрузка классов...</div>
            ) : (
        ...
        <div className="flex justify-center mt-4">
          <span className="font-firenight text-[20px] text-[#FFEED5]">{classOptions.find(c => c.id === selected)?.name || 'Выберите класс'}</span>
        </div>
        <div className="text-[14px] font-jost leading-tight mt-1">
          {(() => {
            const opt = classOptions.find(c => c.id === selected);
            if (!opt) return null;
            return (
              <>
                {opt.summary && <p>{opt.summary}</p>}
                {opt.hitPoints?.hitDie && <p className="mt-1 text-[#DDA852]">Кость хитов: d{opt.hitPoints.hitDie}</p>}
              </>
            );
          })()}
        </div>
...
export default CharacterClassStepMobile;