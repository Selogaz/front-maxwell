...
import { Race, SubRace, CharacterStats, BackgroundOption } from '@/types/character';

interface CharacterOriginStepProps {
  onRandomize?: () => void;
...
// Локальный fallback-маппинг иконки происхождения по slug бэкенда.
...
const CharacterOriginStep: React.FC<CharacterOriginStepProps> = ({
  onRandomize,
  ...
  const selectedOption = backgroundOptions.find((o) => o.id === selected) ?? null;
  const originLabel = selectedOption?.name ?? 'Выберите происхождение';
  const originDescription = selectedOption?.description ?? '';
              ...
              {backgroundOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`Выбрать происхождение: ${option.name}`}
                  >
...
export default CharacterOriginStep;
