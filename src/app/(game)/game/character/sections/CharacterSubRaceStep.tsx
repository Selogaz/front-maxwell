...
import { Race, SubRace, CharacterStats } from '@/types/character';

interface CharacterSubRaceStepProps {
  onRandomize?: () => void;
...
const CharacterSubRaceStep: React.FC<CharacterSubRaceStepProps> = ({
  onRandomize,
  onSelectSubRace,
  race,
  subRace,
  ...
  const handleSelect = (subRaceId: string) => {
    if (onSelectSubRace) {
      onSelectSubRace(subRaceId);
    } else {
      setInternalSelected(subRaceId);
    }
  };

  const displaySubRace = subRace || filteredSubRaces.find(s => s.id === internalSelected);
  const raceLabel = displaySubRace ? displaySubRace.name : race ? 'Нет подрасы' : 'Выберите подрасу';
  const raceDescription = displaySubRace ? displaySubRace.description : race ? 'У этой расы нет подрасы. Нажмите кнопку "Продолжить"' : 'Сначала выберите расу';
  ...
  return (
    <>
      ...
      >
          ...
          {!race ? (
            ...
            <div className="grid grid-cols-3 gap-x-2 gap-y-6 pr-2 pt-5">
              {filteredSubRaces.map((subRaceItem) => {
                const isSelected = subRace ? subRaceItem.id === subRace.id : subRaceItem.id === internalSelected;
                return (
                  <button
                    key={subRaceItem.id}
        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-131 flex flex-col items-center text-center gap-1 max-w-[90%]">
          <span className="font-firenight text-[25px] leading-[130%] text-[#FFEED5] whitespace-normal break-words">
            {raceLabel}
          </span>
          <div className="font-jost font-normal text-[15px] leading-none text-white">
            <p>{raceDescription}</p>
          </div>
        </div>
      </div>
